...
export interface FixationCardPngInput {
  username: string;
  content: string;
  timeHHmm: string;
...
export async function disposeFixationCardBrowser(): Promise<void> {
  if (browserSingleton) {
...
export async function buildFixationCardHtml(
  ...
  html = html.split('../../fonts/Roboto-Variable.ttf').join(embeddedVariableFontDataUrl);
  if (html.includes('{{')) {
    logger.warn('fixation-card template still has unreplaced placeholders');
  }
  return html;
}
...
export async function renderFixationCardPng(input: FixationCardPngInput): Promise<Buffer> {
  ...
  for (const preset of CARD_PRESETS) {
    const html = await buildFixationCardHtml(input, preset.template);
    const rendered = await renderAndMeasureCard(html);
    if (!fallback) fallback = rendered.png;
    logger.info({ preset: preset.name, fits: rendered.fits, ...rendered.metrics }, 'Fixation card preset check');
